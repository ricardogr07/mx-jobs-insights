...
from mexico_linkedin_jobs_portfolio.models import ReportMetrics, PeriodWindow, DimensionCount
...
def create_sample_records() -> tuple[JoinedObservationRecord, ...]:
    ...
    tech_skills = ["Python", "SQL", "AWS", "React", "Java", "Go", "Kubernetes", "ML", "Docker", "Rust"]
    ...
    for seniority_idx, seniority in enumerate(seniorities):
        for skill_idx, skill in enumerate(tech_skills):
            ...
            for count in range(2 + (skill_idx + seniority_idx) % 4):
                job_id_counter += 1
...
def test_seaborn_workflow(pivot):
    ...
    print(f"✓ Converted to base64 PNG")
...
def test_plotly_workflow(pivot):
    ...
    print(f"  (Can be embedded in HTML via <script> tag)")
...
def test_styling_variations():
    ...
    for cmap in colormaps:
        ...
        try:
            fig = create_seaborn_heatmap(pivot, cmap=cmap)
...
def main():
    ...
    try:
        ...
        # Test 3: Plotly
        plotly_fig = test_plotly_workflow(pivot)
