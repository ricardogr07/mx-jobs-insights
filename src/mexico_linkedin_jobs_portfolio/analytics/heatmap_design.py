...
def build_tech_seniority_pivot_from_records(
    ...
    top_n_skills: int = 10,
    top_n_seniorities: int | None = None,
) -> pd.DataFrame:
    ...
    top_skills = sorted(skill_totals.items(), key=lambda x: x[1], reverse=True)[:top_n_skills]
    ...
    seniority_ordered = [s for s in seniority_order if s in seniority_set]
    ...
    for seniority in seniority_ordered:
        data[seniority] = [cross_tab[(tech, seniority)] for tech in top_skill_names]
...
def create_plotly_heatmap(
    ...
    fig = go.Figure(
        data=go.Heatmap(
            ...
            texttemplate="%{text}",
            textfont={"size": 11},
            hovertemplate=("<b>%{y}</b><br>%{x}: " + hover_label + "%{z}<br><extra></extra>"),
            colorbar=dict(
                title="Count",
                thickness=15,
                len=0.7,
...
def seaborn_complete_workflow(
    records: tuple[JoinedObservationRecord, ...],
    output_html_filepath: str | None = None,
) -> str:
