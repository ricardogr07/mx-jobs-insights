...
- Prefer documented repo commands and workflow notes from `codex/config.toml` and `docs/codex/`.
- `codex/config.toml` is a template/catalog only. Do not store secrets, tokens, or machine-specific credentials in it.

This file stays intentionally short. Detailed guidance lives in `docs/codex/`.